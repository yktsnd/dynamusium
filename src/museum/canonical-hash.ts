import type { ContentHash } from './portrait-types.ts';

function rotateRight(value: number, bits: number) {
  return (value >>> bits) | (value << (32 - bits));
}

/** Stable UTF-8 SHA-256 for synchronous browser and worker provenance. */
export function sha256(text: string): ContentHash {
  ...
  return {
    algorithm: 'sha256',
    value: hash.map((word) => word.toString(16).padStart(8, '0')).join(''),
  };
}
...
function normalize(value: unknown): unknown {
  ...
  if (typeof value === 'number' && !Number.isFinite(value)) {
    throw new Error('Canonical scientific inputs must be finite.');
  }
  return value;
}
